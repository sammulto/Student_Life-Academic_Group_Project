...
public class DBUnitTests {
    ...
    @Test
    public void DBconstructorFile(){
        DB db = new DB("testDB", "file");
        assertNotNull("Newly created DB object should not be null", db);
        assertArrayEquals("Newly created DB object should be empty", db.getTasks(), new Task[0]);
    ...
    @Test
    public void insertBaseCaseFile(){
        DB db = new DB("testDB", "file");
        Task t = new Task("idk");
        assertTrue("Insert should return true", db.insertTask(t));
        Task[] tasks = new Task[1];
    ...
    @Test
    public void insertWithDateFile(){
        DB db = new DB("testDB", "file");
        Task t = new Task("idk");
        t.setStartTime("2021-01-10 12:35:20");
        t.setEndTime("2021-01-10 12:25:20");
    ...
    @Test
    public void deleteBaseCaseFile(){
        DB db = new DB("testDB", "file");
        Task t = new Task("idk");
        db.insertTask(t);
        assertTrue("Delete should return true", db.deleteTask(db.getTasks()[0]));
        assertArrayEquals("The database should be empty", db.getTasks(), new Task[0]);

    }

    @Test
    public void updateBaseCaseFile(){
        DB db = new DB("testDB", "file");
        Task t = new Task("idk");
        Task[] tasks = new Task[1];
        db.insertTask(t);
